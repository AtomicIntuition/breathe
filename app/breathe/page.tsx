...
const colorMap: Record<string, { text: string; bg: string; border: string; glow: string }> = {
  arctic: {
    text: 'text-arctic',
    bg: 'bg-arctic/10',
    border: 'hover:border-arctic/40',
    glow: 'hover:shadow-glow-arctic',
  },
  gold: {
    text: 'text-gold',
    bg: 'bg-gold/10',
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="text-slate-light text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
            Military-grade and scientifically-proven techniques for every situation.
          </p>
        </motion.div>

        {/* Category Filter - refined pills */}
        <motion.div
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-200 ${
              selectedCategory === null
                ...
                : 'text-slate-light hover:text-white hover:bg-white/5 border border-transparent'
            }`}
          >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        <h2 className="text-xl md:text-2xl font-semibold tracking-tight">{info.label}</h2>
    ...
    >
      <Link href={`/breathe/${technique.id}`}>
        <Card
          variant="bordered"
          hover
          className={`p-6 h-full relative group ${colors.border} ${colors.glow} transition-all duration-200`}
        >
          {isRecommended && (
            <div className="absolute top-4 right-4">
              <span className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-arctic/15 text-arctic border border-arctic/20 font-medium">
          ...
          {technique.difficulty === 'advanced' && !isRecommended && (
            <div className="absolute top-4 right-4">
              <span className="text-xs px-2.5 py-1 rounded-full bg-orange-500/15 text-orange-400 border border-orange-500/20 font-medium">
                Advanced
              </span>
            </div>
          )}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-lg mb-1 pr-16 text-white">
                {technique.name}
              </h3>
          ...
          <p className="text-sm text-slate-light mt-4 line-clamp-2 leading-relaxed">
            {technique.purpose}
          </p>
        </Card>
      </Link>
